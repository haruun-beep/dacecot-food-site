...
/* Per-guest prices, in cents. These are the money source of truth: the pages
   print them (see .claude/build-experiences.js) and the store charges them, so
   a price change is one edit and the copy cannot drift from the till.

   The Thursday drop-in and the Sunday class both normalise to type 'class' —
   they ARE both classes as far as the admin's Pasta Classes tab is concerned —
   but they are not the same price, and for a long time a drop-in was quietly
   recorded at the Sunday rate. A drop-in is the one carrying drop_in_date. */
const CLASS_PRICE_CENTS = 9500;    // $95.00 per guest — La Domenica, Sunday
const DROP_IN_PRICE_CENTS = 4500;  // $45.00 per guest — Pasta With Erika, Thursday
...
function normalize(payload) {
  ...
  if (type === 'class') {
    ...
    const unit = str(data.drop_in_date) !== '' ? DROP_IN_PRICE_CENTS : CLASS_PRICE_CENTS;
    amount_cents = guests > 0 ? guests * unit : null;
  } else if (type === 'order') {
...
module.exports = { normalize, parsePriceCents, detectType, CLASS_PRICE_CENTS, DROP_IN_PRICE_CENTS };
