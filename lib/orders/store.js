...
const pg = {
  ...
  // null clears the key, so "resume" is a delete rather than a sentinel value.
  async setSetting(key, value) {
    if (value == null) { await pool().query('DELETE FROM site_settings WHERE key = $1', [key]); return null; }
    await pool().query(
...
const local = {
  ...
  async getSetting(key) {
    return readSettings()[key] != null ? String(readSettings()[key]) : null;
  },

  async setSetting(key, value) {
    const all = readSettings();
    if (value == null) delete all[key]; else all[key] = String(value);
    writeSettings(all);
...
module.exports = {
  ...
  getSetting: (...a) => impl().getSetting(...a),
  setSetting: (...a) => impl().setSetting(...a),
